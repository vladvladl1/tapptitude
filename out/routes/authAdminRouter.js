...
//register get
authAdminRouter.get("/register", async (req, res) => {
    res.status(200).send("all good si la register");
});
//login get
authAdminRouter.get("/login", async (req, res) => {
    res.status(200).send("all good si la login");
});
//login post
authAdminRouter.post("/login", async (req, res) => {
    ...
    if (!admin) {
        res.status(401);
        res.send({ error: "nu s-a gasit in baza de date un utilizator cu acest nume" });
    }
    bcrypt.compare(req.body.password, admin.password, (err, resp) => {
        if (err) {
            res.status(400).send({ error: "parola folosita este gresita" });
        }
        if (resp) {
            const token = jwt.sign({ username: req.username }, "aleluia");
